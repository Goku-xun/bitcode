...
- (BTTx *)buildTxWithOutputs:(NSArray *)unspendOuts toAddresses:(NSArray *)addresses amounts:(NSArray *)amounts changeAddress:(NSString *)changeAddress andError:(NSError **)error {
    //获取outs的中的所有的钱。 算出需要找零的钱，加入amounts
    long long allMoney = [BTTxBuilder getAmount:unspendOuts];
    long long value = 0;
    for (NSNumber *amount in amounts) {
        value += [amount unsignedLongLongValue];
    }
    //不这样写可能会越界，负数
    long long minValue = allMoney - [BTTx getMinTransactionForCoin:BTC] - [BTTx getSplitNormalFeeForCoin:BTC];
    if (value < minValue ) {
        NSMutableArray *temp = [NSMutableArray arrayWithArray:amounts];
        uint64_t charge = allMoney - value;
        [temp addObject:[NSNumber numberWithUnsignedInteger:charge]];
        amounts = [temp copy];
    }else{
        *error = [NSError errorWithDomain:ERROR_DOMAIN code:ERR_TX_NOT_ENOUGH_MONEY_CODE
                                 userInfo:@{ERR_TX_NOT_ENOUGH_MONEY_LACK : @(value - [BTTxBuilder getAmount:unspendOuts])}];
...
//    if (value > allMoney) {
//    if (value > allMoney - [BTTx getSplitNormalFeeForCoin:BTC]) {
//
//        *error = [NSError errorWithDomain:ERROR_DOMAIN code:ERR_TX_NOT_ENOUGH_MONEY_CODE
//                                 userInfo:@{ERR_TX_NOT_ENOUGH_MONEY_LACK : @(value - [BTTxBuilder getAmount:unspendOuts])}];
//        return nil;
...
- (BTTx *)buildTxWithOutputs:(NSArray *)outs toAddresses:(NSArray *)addresses amounts:(NSArray *)amounts changeAddress:(NSString *)changeAddress andTx:(BTTx *)tx coin:(Coin)coin{
    ...
    //费率
    uint64_t feeRate = [BTTx getSplitFeeRateForCoin:coin];
...
////        fees = feeBase + (int)(size-400)*0.67;
////        fees = feeBase + (int)(size-400)*1;
//        fees = feeRate * size;
//    }
    fees = feeRate * size;
