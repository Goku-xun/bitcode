...
#define kBTHDAccountLookAheadSize (1)
...
//    BTBIP32Key *internalKey = [self getChainRootKeyFromAccount:accountKey withPathType:INTERNAL_ROOT_PATH];
    BTBIP32Key *internalKey = [self getChainRootKeyFromAccount:accountKey withPathType:EXTERNAL_ROOT_PATH];
...
//        [internalAddresses addObject:externalAddress];
    ...
    internalAddresses = [externalAddresses copy];
    [self wipeHDSeed];
    [self wipeMnemonicSeed];
    if (encryptedMnemonicSeed) {
        /*保存比特币钱包（数据库）。huangwujie*/
...
//    return [self newTxToAddresses:@[toAddress] withAmounts:@[@(amount)] andChangeAddress:[self getNewChangeAddress] password:password andError:error coin:BTC];
    //默认设置一个多余的找零地址
    return [self newTxToAddresses:@[toAddress,self.address] withAmounts:@[@(amount)] andChangeAddress:self.address password:password andError:error coin:BTC];
}


- (BTTx *)getFeesToAddress:(NSString *)toAddress withAmount:(uint64_t)amount andError:(NSError **)error {
    //    return [self newTxToAddresses:@[toAddress] withAmounts:@[@(amount)] andChangeAddress:[self getNewChangeAddress] password:password andError:error coin:BTC];
    //默认设置一个多余的找零地址
    return [self newTxToAddresses:@[toAddress,self.address] withAmounts:@[@(amount)] andChangeAddress:self.address andError:error coin:BTC];
}
...
/*创建一笔失败的交易计算矿工费*/
- (BTTx *)newTxToAddresses:(NSArray *)toAddresses withAmounts:(NSArray *)amounts andChangeAddress:(NSString *)changeAddress andError:(NSError **)error coin:(Coin)coin{
    NSArray *outs;
    if (coin != BTC) {
        outs = [[BTHDAccountAddressProvider instance] getPrevCanSplitOutsByHDAccount:self.hdAccountId coin:coin];
    } else {
...
//    NSArray *signingAddresses = [self getSigningAddressesForInputs:outs];
...
//    return [[BTHDAccountAddressProvider instance] getExternalAddress:self.hdAccountId];
    return [[BTHDAccountAddressProvider instance] getKuarkPayBTCAddress:self.hdAccountId];
...
- (BTBIP32Key *)getAccount:(BTBIP32Key *)master {
    ...
    //临时测试地址
//    BTBIP32Key *coinType = [purpose deriveHardened:1];
...
- (NSArray *)getRelatedAddressesForTx:(BTTx *)tx {
    NSMutableArray *outAddressList = [[tx getOutAddressList]mutableCopy];
