...
- (NSString *)getKuarkPayBTCAddress:(int)hdAccountId; {
    __block NSString *address;
    [[[BTDatabaseManager instance] getTxDbQueue] inDatabase:^(FMDatabase *db) {
        NSString *sql = @"select address from hd_account_addresses"
        " where path_type=? and address_index=? and hd_account_id=? order by address_index asc limit 1  ";
        FMResultSet *resultSet = [db executeQuery:sql, @(EXTERNAL_ROOT_PATH), @(0), @(hdAccountId)];
        if ([resultSet next]) {
            address = [resultSet stringForColumnIndex:0];
        }
        [resultSet close];
...
- (NSArray *)getSigningAddressesByHDAccountId:(int)hdAccountId fromInputs:(NSArray *)inList; {
    KKUTXOModel *model= [KKUTXOModel UnarchiverFileWithFileName:@"KKUTXOModel"];
    NSArray *info = model.data;
    __block NSMutableArray *array = [NSMutableArray new];
#if 0
    [[[BTDatabaseManager instance] getTxDbQueue] inDatabase:^(FMDatabase *db) {
    ...
    for (int i=0; i<info.count; i++) {
        KKUTXOModel *model = info[i];
        BTHDAccountAddress *address = [[BTHDAccountAddress alloc] init];
        address.isSyncedComplete = YES;
        address.isIssued = NO;
        address.hdAccountId = 1;
        address.pathType = EXTERNAL_ROOT_PATH;
        address.index = 0;
...
- (NSArray *)getUnspendOutByHDAccount:(int)hdAccountId; {
    ...
    for (KKUTXOModel *info in model.data) {
        BTOut *outItem = [BTOut new];
        outItem.outSn = (uint)info.outSn;
        outItem.outStatus = 0;
        outItem.hdAccountId = 1;
        outItem.txHash = info.prev_tx_hash;
        outItem.outScript = info.script;
        outItem.outValue = info.outValue;
        outItem.outAddress = info.address;
        outItem.coinDepth = 0;
